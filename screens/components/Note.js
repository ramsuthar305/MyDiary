        ...
        <TouchableOpacity onPress={() => read(item)}>
            <View style={style.GlobalStatus} elevation={3}>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    <Text numberOfLines={1} ellipsizeMode="tail" style={style.menuHeading}>{item.note}</Text>
                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                        <TouchableOpacity onPress={() => edit(item)}>

                            <MaterialCommunityIcons name="pencil-outline" size={22} style={style.icons} color="black" />
                        </TouchableOpacity>
